<template>
  <section id="events" class="section relative overflow-hidden bg-gray-50 py-24">
    <!-- Elementi decorativi avanzati di sfondo -->
    <div
      class="absolute top-0 left-0 w-1/3 h-full bg-blue/5 skew-x-12 transform origin-top-left"
    ></div>
    <div
      class="absolute -bottom-10 -right-10 w-72 h-72 rounded-full bg-red/5 animate-pulse-slow"
    ></div>
    <div class="absolute top-1/4 right-1/4 w-16 h-16 rounded-full bg-red/10 animate-float"></div>
    <div
      class="absolute bottom-1/3 left-1/5 w-12 h-12 rounded-full bg-blue/10 animate-float-delayed"
    ></div>

    <div class="container-custom relative z-10">
      <SectionHeader
        title="Eventi"
        subtitle="Scopri gli eventi e le iniziative che stiamo organizzando per coinvolgere la nostra comunità."
        label="Eventi"
      ...
      <div v-if="loading" class="text-center py-20">
        <div class="relative inline-block">
          ...
          <div
            class="w-16 h-16 rounded-full border-t-4 border-red absolute top-0 left-0 animate-spin-fast"
          ></div>
        </div>
      ...
      <div
        v-else-if="error"
        class="text-center py-16 transform transition-all duration-500 animate-fadeIn"
      >
        <div class="p-8 rounded-xl bg-white shadow-lg max-w-xl mx-auto">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-16 w-16 mx-auto mb-4 text-red"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <button
            @click="fetchEvents"
            class="mt-4 px-4 py-2 bg-red text-white rounded-md hover:bg-red-700 transition-colors"
          >
            Riprova
          </button>
        </div>
      ...
      <div v-else-if="events.length === 0" class="text-center py-16 animate-fadeIn">
        <div class="p-8 rounded-xl bg-white shadow-lg max-w-xl mx-auto">
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        >
          <div class="relative h-64 overflow-hidden">
            ...
            <div
              class="absolute inset-0 border-0 border-red/0 group-hover:border-2 group-hover:border-red/70 transition-all duration-300 rounded-t-xl opacity-0 group-hover:opacity-100"
            ></div>
          </div>
          ...
          <div class="p-8 relative">
            <!-- Status badge -->
            <span
              class="absolute top-0 right-6 -translate-y-1/2 text-xs font-medium bg-red/10 text-red py-1 px-4 rounded-full shadow-sm"
            >
              {{ event.is_featured ? 'In evidenza' : 'Standard' }}
            </span>

            <h3
              class="text-xl font-bold text-gray-800 group-hover:text-red transition-colors duration-300 mb-3"
            >
              {{ event.title }}
            </h3>
            ...
            <div class="pt-4 border-t border-gray-200">
              ...
              >
                ...
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-5 w-5 ml-2 transform group-hover:translate-x-2 transition-transform duration-500"
                  viewBox="0 0 20 20"
                  fill="currentColor"
                >
                  <path
...
const fetchEvents = async () => {
  ...
  try {
    ...
    console.log('Eventi ricevuti:', response.data) // Per debug
    ...
    error.value = 'Errore nel caricamento degli eventi. Riprova più tardi.'
