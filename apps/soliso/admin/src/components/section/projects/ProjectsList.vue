<template>
    ...
    subtitle="Visualizza e gestisci i progetti"
  >
  <div class="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow">
    ...
    <div v-if="!loading && projects.length === 0" class="text-center py-8 text-gray-500">
      ...
      <p class="mt-2">No projects found. Add one!</p>
    </div>

    <div v-if="!loading && projects.length > 0" class="overflow-x-auto -mx-3 sm:mx-0">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                <!-- <th scope="col" class="hidden sm:table-cell px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Description</th> -->
                ...
                <!-- <th scope="col" class="hidden md:table-cell px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Created</th>
                <th scope="col" class="hidden md:table-cell px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Updated</th> -->
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="project in projects" :key="project.id" class="hover:bg-gray-50 transition-colors duration-150">
                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ project.id }}</td>
                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ project.name }}</td>
                <!-- <td class="hidden sm:table-cell px-4 py-3 text-sm text-gray-500 max-w-xs truncate">{{ project.description || '-' }}</td> -->
                <td class="px-3 sm:px-4 py-3 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0">
                      <ProjectImage :image-url="project.thumbnail_url" :alt-text="project.name" size="150x150" />
                    </div>
                  </div>
                </td>
                <td class="px-3 sm:px-4 py-3 whitespace-nowrap">
                ...
                <td class="hidden sm:table-cell px-4 py-3 whitespace-nowrap">
                  <ActiveBadge :is-active="project.is_active" />
                </td>
                <!-- <td class="hidden md:table-cell px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ formatDate(project.created_at) }}</td>
                <td class="hidden md:table-cell px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ formatDate(project.updated_at) }}</td> -->
                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-center text-sm font-medium">
                  <div class="flex justify-center space-x-2">
                    ...
                    <button @click="confirmDelete(project.id)" class="text-red-500 hover:text-red-700 transition-colors duration-150 p-1 rounded-full hover:bg-red-50" title="Delete">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
import ProjectImage from '../../../components/ui/images/ProjectImage.vue';
...
const confirmDelete = (id) => {
  if (window.confirm(`Are you sure you want to delete project ${id}? This action cannot be undone.`)) {
    deleteProject(id);
  }
};
...
const deleteProject = async (id) => {
  try {
    ...
    error.value = `Failed to delete project ${id}.`;
    if (err.response && err.response.data && err.response.data.detail) {
