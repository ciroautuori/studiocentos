    ...
    <div class="border-b border-gray-200 bg-gray-50 px-4 py-5 sm:px-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="mt-4 flex flex-col sm:mt-0 sm:flex-row sm:space-x-3 space-y-3 sm:space-y-0">
          <router-link v-if="project" :to="{ name: 'project-edit', params: { id: project.id } }"
      ...
      <div v-if="!loading && project" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="px-4 py-5 sm:p-6">
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <dd class="mt-1 text-gray-900">{{ formatDate(project.created_at) }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500">Updated At</dt>
                    <dd class="mt-1 text-gray-900">{{ formatDate(project.updated_at) }}</dd>
                  </div>
                </dl>
              </div>
            </div>
          ...
          <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <div class="px-4 py-5 sm:p-6">
              <ProjectImage :image-url="project.thumbnail_url" :alt-text="project.name" />
            </div>
          </div>
        </div>
      </div>
